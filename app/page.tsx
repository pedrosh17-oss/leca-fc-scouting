...
import { 
  ...
  Filter, Zap, Crosshair, BrainCircuit
} from 'lucide-react';
...
export default function Home() {
  ...
  // Lógica de Jogadores (Pesquisa + Limite Inicial)
  ...
  // Se não houver pesquisa, mostra apenas os primeiros 30
  const displayedPlayers = search ? filteredPlayers : filteredPlayers.slice(0, 30);

  // Lógica de Equipas (Filtros + Pesquisa)
  ...
  return (
    <main className="min-h-screen bg-[#0d131f] text-slate-100 p-6 font-sans">
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div>
            ...
            <div className="flex justify-between items-center mb-4">
              <span className="text-sm text-slate-400">
                A mostrar {displayedPlayers.length} de {filteredPlayers.length} atletas.
                {!search && " Utilize a pesquisa para ver mais."}
              </span>
            </div>
            ...
            <div className="grid gap-3">
              {displayedPlayers.map((player) => (
                <div key={player.id} className="bg-[#151c2c] border border-slate-800/80 rounded-xl p-4 flex items-center justify-between hover:border-slate-700 transition">
                  ...
                  <button onClick={() => { setSelectedPlayer(player); setProfileTab('timeline'); }} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium rounded-lg transition border border-slate-700">
                    Ver Perfil Completo
                  </button>
                </div>
              ))}
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">
              ...
              <div className="flex gap-4">
                <select 
                  ...
                  onChange={(e) => setTeamFilterComp(e.target.value)} 
                  className="bg-[#151c2c] border border-slate-800 rounded-xl py-3 px-4 text-sm text-slate-200 focus:outline-none focus:border-blue-500"
                >
        ...
        {/* TAB 3: MATCHES (COM NOVOS DADOS TÁTICOS) */}
        {activeTab === 'matches' && (
                    ...
                    {isExpanded && (
                      <div className="p-5 border-t border-slate-800 bg-[#111723] space-y-5">
                        <div className="flex flex-wrap items-center justify-between gap-4 text-xs text-slate-300 bg-[#0d131f] p-4 rounded-lg border border-slate-800">
                          <div className="flex items-center gap-2"><UserCheck size={16} className="text-blue-400" /> <span><strong>Scout:</strong> {match.scout}</span></div>
                          <div className="flex items-center gap-2"><Users size={16} className="text-emerald-400" /> <span><strong>Atletas Referenciados:</strong> {match.playersList}</span></div>
                        </div>
        ...
        {/* TAB 4: SCOUTS (NOVO VISUAL) */}
        {activeTab === 'scouts' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {scouts.map((scout) => (
              <div key={scout.id} className="bg-[#151c2c] border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition">
                <div className="flex items-center gap-4 border-b border-slate-800/60 pb-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-400 font-medium mt-0.5 line-clamp-1">{scout.competitions}</p>
                  </div>
                </div>
      ...
      {/* MANTÉM OS MODAIS (selectedPlayer e selectedTeam) INTACTOS AQUI - NÃO PRECISAMOS DE MUDAR NADA NELES */}
