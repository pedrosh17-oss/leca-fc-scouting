...
import { 
  Trophy, Plus, Calendar, Shield, UserCheck, ChevronUp, ChevronDown, 
  Edit3, Search, ExternalLink, Loader2, Zap, Activity, Crosshair, BrainCircuit 
} from 'lucide-react';
import CustomSelect from '../ui/CustomSelect';
import CustomMultiSelect from '../ui/CustomMultiSelect';
import { Player, Scout } from '../../types';
...
interface MatchesTabProps {
  ...
  startEditMatchContext: (match: any) => void;
  setExpandedMatchEdit: (id: string | null) => void;
  reportData: any;
  setReportData: React.Dispatch<React.SetStateAction<any>>;
  handleReportSubmit: (matchId: string) => Promise<void>;
  submittingReport: boolean;
  setIsAddHighlightOpen: (val: { matchId: string; matchName: string } | null) => void;
  setNewHighlightData: React.Dispatch<React.SetStateAction<{ playerId: string; notes: string }>>;
  setEditingHighlight: (val: { matchId: string; matchName: string; player: any; highlightId: string | null; notes: string } | null) => void;
  ...
  setPreGameData: React.Dispatch<React.SetStateAction<any>>;
  preGameData: any;
...
export default function MatchesTab({
  ...
  toggleMatch,
  editingMatchId,
  startEditMatchContext,
  setExpandedMatchEdit,
  reportData,
  setReportData,
  handleReportSubmit,
  submittingReport,
  setIsAddHighlightOpen,
  setNewHighlightData,
  setEditingHighlight,
  setSelectedPlayer,
  setProfileTab,
  ...
  const handleOpenScheduleModal = () => {
    setPreGameData({ 
      ...
      competitionId: '', 
      scoutIds: authScoutId ? [authScoutId] : [], 
      type: '' 
    });
  ...
  return (
    <div className="animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5 md:mb-6">
        <p className={`text-xs md:text-sm ${themeTextMuted} hidden sm:block`}>
          Motor de observação de equipas e atletas.
        </p>
        {canCreateMatches && (
          <button 
            onClick={handleOpenScheduleModal} 
            className="w-full sm:w-auto flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-5 py-4 md:py-3 rounded-xl text-sm md:text-base font-bold transition shadow-lg shadow-blue-900/20"
          >
            ...
            <div key={match.id} id={`match-${match.id}`} className={`${themeCard} border ${isExpanded ? 'border-blue-500/50' : ''} rounded-xl overflow-hidden transition-all duration-300 shadow-sm`}>
              <div onClick={() => toggleMatch(match.id)} className={`p-4 md:p-5 flex items-center justify-between cursor-pointer ${isDarkMode ? 'hover:bg-slate-800/40' : 'hover:bg-slate-50'}`}>
                <div className="flex items-center gap-3 md:gap-4 min-w-0">
                  <div className={`p-3 rounded-xl ${isExpanded ? 'bg-blue-500 text-white' : 'bg-blue-500/10 text-blue-500'} border border-blue-500/20 flex-shrink-0 transition-colors`}>
                ...
                <div className="flex items-center gap-3 md:gap-6 flex-shrink-0 pl-2">
                  ...
                  <div className={`text-slate-400 ${isDarkMode ? 'bg-slate-800/50' : 'bg-slate-100'} p-2 md:p-1.5 rounded-lg`}>
                ...
                <div className={`p-4 md:p-5 border-t ${isDarkMode ? 'border-slate-800 bg-[#0d131f]' : 'border-slate-200 bg-slate-50'} space-y-6 md:space-y-5`}>
                  ...
                  <div className={`${themeCard} border p-4 rounded-xl text-xs md:text-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4`}>
                    <div className="space-y-2 md:space-y-1.5 w-full md:w-auto">
                      ...
                      <div className="flex items-center gap-2"><UserCheck className="w-4 h-4 text-slate-400"/> <strong>Scout:</strong> <span className="text-blue-500 font-medium">{match.scout || 'N/D'}</span></div>
                    </div>
                    {canEditMatches && (
